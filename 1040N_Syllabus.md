# MATH 1040N Syllabus - Fall 2026

## Contact Information

| __Instructor__      | __Dr. Michael E. Olson__                                           |
| :------------------ | :----------------------------------------------------------------- |
| __Email__           | michael.olson2@snow.edu                                            |
| __Office Hours__    | 2:00 pm - 3:00 pm Monday-Thursday (in person) and<br>3:00 pm - 4:00 pm Monday-Thursday (online) or by appointment                |
| __Office__          | GRSC 312, Ephraim Campus                                           |
| __Microsoft Teams__ | Instant Messaging & Video Calls available through Microsoft Teams - Use my @snow.edu address |

[My Office Schedule](https://docs.google.com/spreadsheets/d/1l6Z5ndgsixhoEFm65yo8SgkMB8Tv8KQTkz2CSJVxPOE/edit?usp=sharing)


## Required Materials
1. [__Textbook__](https://drolsonmi.github.io/math1040/Resources/OpenIntroTextbook.pdf)
    - [List of reading assignments](https://drolsonmi.github.io/math1040/Resources/TextbookSections.html)
2. __Statistical Calculator__
    - I recommend the TI-83 or TI-84. These calculators perform all the calculations we will discuss this semester.
    - If you have another calculator, you will be responsible for learning how to use it (The TI-89 and the TI-Nspire are not allowed)
    - Other calculators may calculate some variables differently, so you will need to be aware of these different functions and how to do it by hand
3. __Microsoft Excel__
    * Students at Snow College have free access to Microsoft 365 apps
    * For more information, check out the [snow.edu IT page on Microsoft 365](https://snow.edu/offices/it/o365.html).
        - To access, go to [my.snow.edu](https://my.snow.edu) then find "Office 365" in the Quick Links section
        - Alternatively, you can go directly to [office.com](https://www.office.com/) and log in with your @students.snow.edu address

### Optional Materials
* Microsoft Teams (website or app on the phone)
    * teams.microsoft.com
    * Log in using your Snow College email

* The Canvas App on your phone

## How to succeed in this course
I recommend doing the following through the semester. Doing so will give you the greatest chance of succeeding this semester. In addition to that, it will cause far less stress.

The key is to plan ahead.
* Look up all homework assignment due dates
* Look up all project dates
* Look up all exam dates

If you have any trips planned, plan ahead and complete all assignments before you go on your trip. You are responsible for your own grade.

Do not plan on extra credit. As you see later in the extra credit policy, I will give extra credit as appropriate in the class. I will never give extra credit just to give anyone more points. So, make sure you complete your homework on time so that you don’t depend on extra credit.

Check your grade early and often. If you wait until after the withdrawal deadline to ask questions about your grade, then it is unlikely that I can help you raise your grade in time for the end of the course. So, ask questions early.

## Grading Policy
The purpose of grades is to show me that you understand the material we discussed throughout the semester. The grading policy has been built in a way to give you the opportunity to demonstrate what you have learned.

Some students do well with exams. Others do better with projects. As such, I will grade everyone in two different ways:

| Assignment | Percentage of Grade<br>(Method 1) | Percentage of Grade<br>(Method 2) |
| :--------: | :-------------------------------: | :-------------------------------: |
| Homework   | 25%                               | 20%                               |
| Projects   | 20%                               | 15%                               |
| Exams      | 35%                               | 40%                               |
| Final Exam | 20%                               | 25%                               |

I will grade everyone using both methods, and you will be rewarded the higher grade.

### Overview of Course Units
The course will be separated into 5 segments:
1. Basic Statistical Studies (setting up studies and graphing data)
2. Analysis of Numerical Data
3. Analysis of Categorical Data
4. Central Limit Theorem and Confidence Intervals
5. Hypothesis Testing

### Assignment Overview
__Homework__ assignments will be assigned weekly. There will be 2 or 3 homework assignments that cover the material discussed throughout the week. Each lesson has an "Individual Preparation Assignment" and a "Homework Assignment". All assignments will be due by the end of the Saturday of the week assigned.

Throughout the semester, you will participate in multiple __projects__ to improve in your skills. These will be completed weekly and correspond with material studied through the week. The intent of these projects is to give a realistic scenario to apply what you have learned.

__Exams__ will be given periodically through the semester to give you opportunity to show your understanding. The __final exam__ will be given in the week after the end of classes as indicated on the course schedule.
* The midterm and final exams need to be scheduled with a proctor or a Snow College Testing Center at least one week ahead of the exam date. (For example, if the exam is on February 21, you should have a time scheduled by February 14.)

### Late Policy
* Late homework and projects will receive 70% credit for 1 week after the due date. No credit will be given if it’s more than 1 week late.
* Exams cannot be made up. If you know you will miss an exam, contact me at least a week before the exam to schedule another time to take the exam.

Throughout the semester, I allow 3 late assignment submissions throughout the semester with no questions asked. These late submissions can be applied to:
* homework
* projects

If you would like to use one, just let me know you would like to use a late submission and what assignment or project you would like to use it on. I will then go in and extend the due date for another week so you have time to complete it for full credit.

All late assignments must be approved by two weeks before the last day of the semester.

The last day to turn in late assignments is one week before the last day of the semester.

### Extra Credit Policy
As your instructor, my duty is to teach you and to achieve the outcomes outlined above. As such, I teach this course with a goal of understanding the material and satisfying those outcomes.

The concept of extra credit demonstrates that the goal is not to understand, but to just earn enough points to make it through. Extra credit undermines the goals of the course.

As such, *I will not award extra credit for the sake of getting more points*. If I see a need through the semester and as appropriate for the course, I will add extra assignments as needed, but with the express purpose of helping to understand difficult topics.

There is no need to ask if I will offer extra credit. If I do, I will inform you.

### Academic Integrity
You have paid me to learn everything you can. Academic dishonesty is thus a waste of the time and money you have committed to your education. Additionally, dishonest performance will *ruin* your future careers (assuming, of course, that you can get a career). I do not tolerate dishonesty in the classroom.

On the other hand, I know that there are incalculable resources to help you understand the class material. I have no problem with you using internet resources to help you learn as you complete homework and projects. However, exams are meant for you to show me how much you have learned. As such, I encourage you to follow this process:
- Try homework and projects on your own as much as possible
    - Use internet resources to learn about anything that causes confusion
    - Practice what you have learned (you can ask AI (in study mode) to create practice problems)
    - Complete the homework and projects
- When practicing for the exam, practice without assistance as much as possible
    - Use internet resources to learn about anything that causes confusion
    - Complete practice questions (from reviews or from AI in study mode) with as little assistance as possible
    - Continue practicing until you can solve problems without assistance

#### Using AI in this class
Generative AI is a technology that automates part or all of the writing and content creation process. Popular examples include ChatGPT (OpenAI), Gemini (Google), Claude (Anthropic), and others. Statistics is actually the core of Generative AI. Computers use statistics and probabilities to determine output text. 

Throughout the semester, you may only use generative AI when I provide specific direction to do so. We will use generative AI in a guided way to ensure that course learning outcomes and pedagogy are fully supported.

As the instructor, I reserve the right to disqualify homework, project, and exam scores from the grade if there is evidence of improper AI use. If improper AI use is suspected, I may call you in to discuss the situation. __Remember that you are in charge of your own education__. Do not cheat yourself just to make life easy. You won't have that chance in the professional world.

#### Consequences for cheating
Cheating for this class is considered to be using any resources to complete assignments or exams without actually understanding what is done. Examples of cheating include, but are not limited to,
- Copying another students' work
- Using AI to complete the assignment
- Bringing extra notes that aren't allowed into exams

Consequences for cheating are:
- Homework and Projects
    - Alerts and discussions with the professor
    - 0 for assignments if the problems persist
- Exams
    - Depending on the evidence and the severity of the offense, consequences will vary from a 0 on the exam to a 0 for the entire course or expulsion/suspension from the college.
    - There will be absolutely NO tolerance for cheating on the final exam. Cheating on the final exam will result in a failing grade for the course.
- The professor has the right to determine other consequences based on the circumstances

Your job in this class is to learn. The purpose of exams is to show me how much you have learned. Don't sacrifice your entire future by dishonesty. If you are concerned about upcoming exams, come talk to me and study.

### Snow College Policies
Click the links below for college-wide policies.
* [Academic Integrity](https://snow.instructure.com/courses/474797#academic-integrity). Avoiding cheating and how acts of academic dishonesty are handled.
* [Accommodations through the Office of Disability Services](https://snow.instructure.com/courses/474797#ada). How to arrange for help due to a disability.
* [Attendance](https://snow.instructure.com/courses/474797#attendance). How non-attendance can affect your success in a class.
* [Belonging and Support](https://snow.instructure.com/courses/474797#belonging). We want you to succeed.
* [Family Educational Rights and Privacy Act](https://snow.instructure.com/courses/474797#ferpa). Detailing your privacy rights at school.
* [Artificial Intelligence](https://snow.instructure.com/courses/474797#ai). How to use and not use AI.
* [Nondiscrimination](https://snow.instructure.com/courses/474797#nondiscrimination). Understanding your rights.
* [Religious Observance](https://snow.instructure.com/courses/474797#religious-observances). How to navigate a religious holiday interfering with your classes.
* [Safety and Title IX](https://snow.instructure.com/courses/474797#safety). Safety and sexual discrimination and assault.
* [Sensitive Course Content](https://snow.instructure.com/courses/474797#content). How to handle difficult course material.
