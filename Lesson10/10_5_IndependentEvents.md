...
To check whether two events are independent, compare the conditional probability $P(B\lvert A)$ to the plain probability $P(B)$:
 
- If $P(B\lvert A) = P(B)$, the events are **independent**.
...
When two events are independent, the general multiplication rule from 10.4 simplifies, since $P(B\lvert A)$ is just $P(B)$:
 
$$P(A~AND~B) = P(A)\cdot P(B\lvert A) \qquad\Longrightarrow\qquad P(A~AND~B) = P(A)\cdot P(B) \quad \text{(independent events only)}$$
