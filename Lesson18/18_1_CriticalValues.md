...
2. Type `=NORM.S.INV(area)`, replacing `area` with the area of the left tail, then press Enter.
3. This returns the critical value on the left side of the distribution. Because the standard normal distribution is symmetric, the critical value on the right is the same value made positive.
For example, for a 90% confidence level, the left tail has an area of 0.05. Typing `=NORM.S.INV(0.05)` returns approximately -1.645, so the critical value is $\pm 1.645$.
